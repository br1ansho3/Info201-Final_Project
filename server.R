...
nutrition_table <- function(dish) {
  ...
  nutrition_response_text <- content(nutrition_response, type = "text",
                                     encoding = "ISO-8859-1")
...
server <- function(input, output, session) {
  ...
  recipe_input <- eventReactive(input$search, {
    test <- paste(input$ingredient_1, input$ingredient_2, input$ingredient_3,
                  input$ingredient_4, input$ingredient_5, sep = ",")
    while (substr(test, nchar(test), nchar(test)) == ",") {
      test <- gsub(".$", "", test)
    }
    param <- list(test)
    ingredients <- list(ingredients = param, number = input$amount)
    response <- GET(paste0(recipe_uri, recipe_endpoint),
                    add_headers("X-RapidAPI-Key" = recipe_key),
                    query = ingredients)
